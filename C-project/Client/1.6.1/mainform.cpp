...
mainform::mainform(QWidget *parent) : QMainWindow(parent)
{
    ...
    friendsname->resize(150, 50);
    friendsname->move(420, 50);
...
void mainform::successlogin()
{
    ...
    out<<name;
    socket1->write(array);
    QHostAddress address = QHostAddress("47.107.48.106");
    quint16 p = 6660;
    QByteArray array2;
    QDataStream out2(&array2, QIODevice::WriteOnly);
    out2<<1;
    out2<<name;
    usocket2->writeDatagram(array2, address, p);
}
...
void mainform::send()
{
    ...
    if(!friendonline){
        ...
        out<<msg;
        quint16 port2 = map2[msg.receiver];
        usocket1->writeDatagram(array, Address, port2);
    }
...
void mainform::receive()
{
    ...
    if(iscommunicate){
        displaytext->append("[" + msg.sender + "]" + msg.time);
...
void mainform::communicate(QListWidgetItem *item)
{
    ...
    displaytext->clear();
    friendonline = item->backgroundColor() ==  QColor(205,1,20,255) ? false : true;

    if(friendonline && item->backgroundColor() == QColor(0,255,0,255))
        item->setBackgroundColor(QColor(255, 255, 20, 255));
    if(!friendonline && item->backgroundColor() == QColor(0,255,0,255))
        item->setBackgroundColor(QColor(205, 1, 20, 255));
